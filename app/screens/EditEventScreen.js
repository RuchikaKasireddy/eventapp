...
export default function EditEventScreen({route, navigation}) {
            ...
            if(title && description && venue && dept && date && time && image){
                ...
                uploadBytes(storageRef, blob).then((snapshot)=>{
                    getDownloadURL(snapshot.ref).then(url=>{
                        ...
                        set(dbRef(db, `events/${id}`), dataObj).then(()=>{
                            setUploading(false)
                            ...
                            Alert.alert('Event Update Error', 'Failed to update event', [
                                {
                                    text: 'OK'
                                }
                            ])
                ...
                setUploading(false)
                Alert.alert('Event Update Form', 'Form is not sufficiently filled', [
                    {
                        text: 'OK'
                    }
                ])
    ...
    useEffect(()=>{
        setTitle(route.params.details?.title ?? '')
        setDescription(route.params.details?.description ?? '')
        setVenue(route.params.details?.venue ?? '')
        setDept(route.params.details?.organizer ?? null)
        setDate(route.params.details?.date ?? '')
        setTime(route.params.details?.time ?? '')
        setImgUrl(route.params.details?.img ?? '')
                    ...
                    <View>
                        ...
                        <TouchableOpacity onPress={updateEvent}>
                            <View style={styles.createButton}>
                                {!uploading ? <Text style={styles.createText}>Update Event</Text> : <ActivityIndicator size="large" color={colors.white} />}
                            </View>
                        </TouchableOpacity>
